...
urlpatterns = [
   ...
   # path('logout/',auth_views.LogoutView.as_view(success_url = reverse_lazy('home')),name="logout"),
    ...
    path('register/',views.NewUserView.as_view(template_name="cutomuser/newuser.html",form_class=forms.UserCreationForm),name='register'),
   # path('apply-for/<j_pk>/',views.JobAppllicationView.as_view(template_name="cutomuser/job_application.html",form_class=forms.ApplicationForm),name='apply'),
   path('apply-for/<job_id>/',views.create_job_application,name='apply'),
